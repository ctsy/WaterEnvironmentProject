...
function GetQueryStringNotDecode(name) {
    var reg = new RegExp("(^|&)" + name + "=([^&]*)(&|$)", "i");
    var r = window.location.search.substr(1).match(reg);
...
function Dialog(content, options) {
    ...
    //options.id = options.id ? options.id : options.idname; // 唯一ID
...
//        dialog.width(defaults.width);
//        dialog.height(defaults.heigth);
        if (options.center) {
    ...
    */
    var init = function () {
        ...
        dialog.css({ 'z-index': ++Dialog.__zindex, 'position': options.fixed ? 'fixed' : 'absolute', 'opacity': ++options.opacity });
